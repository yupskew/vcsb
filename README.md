...
| `PREFIX`    | The prefix that triggers a command. Defaults to `!` if not set.                                  |
...
There are two types of commands in this bot — **global commands** and **per-account commands**.

### Global Commands

These commands affect **all** accounts at once.
...
5. They join with audio and microphone enabled (not deafened, not muted).
